...
GRAYSCALE=True; IMG_SIZE=84; STACK=4; NORM_MEAN=0.5; NORM_STD=0.5
    ...
    for ep in range(3):
        ...
        while not done:
            ...
            while len(frames) < STACK:
                frames.insert(0, x)
            frames = frames[-STACK:]
            x_stacked = np.concatenate(frames, axis=0)
        ...
        print(f"Episodio {ep+1} → Recompensa total: {total_reward:.1f} en {steps} pasos")
